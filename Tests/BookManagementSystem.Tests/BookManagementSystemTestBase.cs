...
{
    public abstract class BookManagementSystemTestBase : AbpIntegratedTestBase<BookManagementSystemTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BookManagementSystemTestBase()
        {
        ...
        protected void UsingDbContext(Action<BookManagementSystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BookManagementSystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BookManagementSystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BookManagementSystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BookManagementSystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BookManagementSystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BookManagementSystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BookManagementSystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BookManagementSystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BookManagementSystemDbContext, Task<T>> func)
        {
            T result;
